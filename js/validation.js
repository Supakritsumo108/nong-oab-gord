// ไฟล์: js/validation.js (ใหม่ - ระบบ Validation แยกไฟล์)
...
export function validateEmail(email) {
  ...
  // ✅ (ตัวเลือก) ตรวจสอบ BCN domain
  // if (!email.endsWith('@bcn.ac.th')) {
  //   return { valid: false, message: "กรุณาใช้อีเมล @bcn.ac.th" };
  // }
...
export function validateUsername(name) {
  ...
  // ตรวจสอบว่าเป็นภาษาไทยหรืออังกฤษเท่านั้น
...
export function validateImageFile(file, maxSizeMB = 2) {
  if (!file) {
    return { valid: true, message: "" }; // ไม่บังคับ
  }
